...
model.rep.plot = function(N, beta, I.0, p, epsilon, theta, gamma, TimeH, TimeNH, reps = 10){
  line.plot = function(data){
    ...
    list(geom_line(data = data, aes(x = Day, y = Infected, color = "Daily Infected"), size = size),
         ...
         scale_color_manual(values = c("Daily Infected" = "red", 
                                      ...
                                      "Daily Hospitalized" = "darkorchid1", 
  ...
  plot = ggplot() + labs(y = "Population", title = title, color = "Compartment") + theme_minimal()
  for(i in 1:reps){
    data = model.data(N, beta, I.0, p, epsilon, theta, gamma, TimeH, TimeNH)
...
a + annotate(geom = "curve", xend = 65, yend = 1600, x = 73, y = 400, curvature = .2, arrow = arrow(length = unit(1, "mm")), size=0.7, alpha=0.6) +
  annotate(geom = "text", x = 57, y = 1800, label = "Total Death", hjust = "left") + 
  
  annotate(geom = "curve", xend = 24, yend = 2150, x = 13.5, y = 550, curvature = -.2, arrow = arrow(length = unit(2, "mm")), color = "blue", size=0.7, alpha=0.6) +
  annotate(geom = "text", x = 24, y = 2300, label = "Max Daily New Case", hjust = "left", color = "blue") + 
  ...
  annotate(geom = "curve", xend = 30, yend = 600, x = 18, y = 110, curvature = -.2, arrow = arrow(length = unit(2, "mm")), color = "darkorchid1", size=0.7, alpha=0.6) + 
  annotate(geom = "text", x = 31, y = 650, label = "Max Daily Hospitalized", hjust = "left", color = "darkorchid1") +  theme(legend.position="bottom")
b = model.rep.plot(10000, 2, 5, 0.5, 0.05, 0.9, 0.9, 10, 5)
b + annotate(geom = "curve", xend = 70, yend = 1000, x = 62, y = 600, curvature = -.2, arrow = arrow(length = unit(1, "mm")), size=0.7, alpha=0.6) +
  annotate(geom = "text", x = 71, y = 1050, label = "Total Death", hjust = "left") + 
  
  annotate(geom = "curve", xend = 24, yend = 2400, x = 11, y = 1070, curvature = -.2, arrow = arrow(length = unit(2, "mm")), color = "blue", size=0.7, alpha=0.6) +
  annotate(geom = "text", x = 24, y = 2600, label = "Max Daily New Case", hjust = "left", color = "blue") + 
  ...
  annotate(geom = "curve", xend = 30, yend = 900, x = 14, y = 230, curvature = -.2, arrow = arrow(length = unit(2, "mm")), color = "darkorchid1", size=0.7, alpha=0.6) + 
  annotate(geom = "text", x = 31, y = 1000, label = "Max Daily Hospitalized", hjust = "left", color = "darkorchid1")  +  theme(legend.position="bottom")
```
...
Infected.Lm = linear("InfectedProportion")
summary(Infected.Lm)
...
MaxCase.Lm = linear("MaxCaseProportion")
summary(MaxCase.Lm)
...
MaxHospital.Lm = linear("MaxHospitalProportion")
summary(MaxHospital.Lm)

...
model.data = function(N, beta, I.0, p, epsilon, theta, gamma, TimeH, TimeNH){
  ...
  while(Total.Infective * S > 0 & curr.day < 1000){
    curr.day = curr.day + 1
...
```{r}
test.model = SIRHD.model(10000, 4, 5, 0.5, 0.2, 0.9, 0.9, 10, 5)
test.model[[1]]
test.model[[2]]
...
Death.Anova1 = aov(DeathProportion ~ Beta + HospitalizedRecoveryRate + NonHospitalizedRecoveryRate + InfectionTime 
                   + Beta*HospitalizedRecoveryRate + Beta*NonHospitalizedRecoveryRate + Beta*InfectionTime 
                   + HospitalizedRecoveryRate*NonHospitalizedRecoveryRate + HospitalizedRecoveryRate*InfectionTime + NonHospitalizedRecoveryRate*InfectionTime, 
                   data = result_table)
...
avg.HospitalizedRecoveryRate$Var = "Hospitalization Recovery Rate"
names(avg.HospitalizedRecoveryRate) = c("Level", "Average.Death.Proportion", "Var")

avg.NonHospitalizedRecoveryRate$Var = "Non-Hospitalization Recovery Rate"
...
avg.all = rbind(avg.Beta, avg.HospitalizedRecoveryRate, avg.NonHospitalizedRecoveryRate, avg.InfectionTime)
...
ggplot(avg.all, aes(x = Level, y = Average.Death.Proportion, group = Var)) + 
  ...
  geom_hline(yintercept = mean(result_table$DeathProportion))

# Summarize interaction effects by all combinations of significant variables
DeathSummary = result_table %>% group_by(Beta, InfectionTime, HospitalizedRecoveryRate, NonHospitalizedRecoveryRate) %>% summarise(Average.Death.Proportion = mean(DeathProportion))
...
ggplot(DeathSummary, 
  ...
  geom_point() + 
  geom_line() +
  facet_grid(Beta ~ InfectionTime, labeller = labeller(InfectionTime = InfectionTime.labs, Beta = Beta.labs)) +
...
ggplot(InfectedSummary, 
       aes(x = HospitalizationRate, 
           y = Average.Infected.Proportion, 
           group = Beta,
           color = Beta)) + 
  geom_line() +
  geom_point() + 
  facet_grid(~ InfectionTime, labeller = labeller(InfectionTime = InfectionTime.labs, Beta = Beta.labs)) +
  theme_bw() + 
  ggtitle("Interaction Plot for Infected Proportion")
```
...
```{r}
Hospital.Anova = aov(MaxHospitalProportion ~ Population + Beta + HospitalizationRate + HospitalizedRecoveryRate + NonHospitalizedRecoveryRate + HospitalizationTime + InfectionTime + Beta:InfectionTime, data = result_table)
summary(Hospital.Anova)
```
